...
| DESCRIPTION                                                                    |
| Checks for files on disk based on their path and hash. If they exist, collects |
| basic information including path, timestamps, hashes, and size.                |
|                                                                                |
| VARIABLES                                                                      |
...
ON f.path = h.path
WHERE (f.path LIKE '$$path1$$' OR f.path LIKE '$$path2$$' OR f.path LIKE '$$path3$$') AND h.$$hash_type$$ LIKE '$$hash$$' AND f.filename != '.'