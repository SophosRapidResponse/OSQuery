...
| DESCRIPTION                                                                    |
| This query collects all event ID 4624 and 4625 login events from the Security  |
| event log. It includes blank columns so the data can be easily combined with   |
| the other queries in this section that match the same format.                  |
|                                                                                |
| VARIABLES                                                                      |
| - username (type: username) = username to search for                           |
| - source_ip (type: IP address) = source IP user is logging in from             |
| - workstation_name(type: Device Name) = Computer on which the login was started|
| - logon_type (type: string) = logon type to search for                         |
|                                                                                |
| TIP                                                                            |
...
AND JSON_EXTRACT(data, '$.EventData.LogonType') LIKE '$$logon_type$$'
AND JSON_EXTRACT(data, '$.EventData.WorkstationName') LIKE '$$workstation_name$$'
AND time > 0
ORDER BY time DESC